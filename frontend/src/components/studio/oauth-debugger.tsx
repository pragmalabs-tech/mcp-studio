...
function deriveStepStatuses(
  ...
  // Once connected, authorize + token_exchange necessarily succeeded.
  // Authorization has no event type (pure redirect); token_exchange events
  // are emitted in the callback tab and don't reach the original Studio tab.
  if (oauthStatus === "connected") {
    if (statuses["authorization"] === "idle")
      statuses["authorization"] = "success";
    if (statuses["token_exchange"] === "idle")
      statuses["token_exchange"] = "success";
  }
        ...
        <div className="mt-2 bg-red-500/10 border border-red-500/20 rounded px-2 py-1.5">
          <p className="text-[10px] text-red-400">
            ...
            {events
              .filter(
                (e) =>
                  e.step === FLOW_STEPS[failedIndex].key &&
                  e.status === "error",
              )
              .slice(-1)
              .map(
